<script lang='ts'>
    ...
    import InventoryItemCard from './InventoryItemCard.svelte'; // ADDED: New component
    ...
    let inventoryViewMode = 'list'; // NEW STATE: 'list' (default table) or 'grid' (card view)
...
<div class="container mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div id="inventory-tabs" role="tablist" class="tabs tabs-bordered justify-center">
        ...
        <button role="tab" class="tab" class:tab-active={activeTab === 
        'locations'} on:click={() => activeTab = 'locations'}>Locations</button>
                            ...
                            <table class="table w-full">
                                ...
                                <tbody>
								    {#each $inventory as item (item.id)} <tr class="hover">
										    ...
										    <td class="font-greycliffmed">{item.name ||
                                            'N/A'}</td>
                                            <td>{ $locations.find(loc => loc.id === item.locationId)?.name ||
                                            'N/A' }</td>
                                            <td><span class="badge badge-ghost badge-sm">{item.type ||
                                            'N/A'}</span></td>

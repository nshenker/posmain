        ...
        <div class="py-4 grid grid-cols-2 md:grid-cols-3 gap-4 max-h-96 overflow-y-auto">
            {#each themes as themeOption}
                <div 
                    ...
                    class="border-2 rounded-xl cursor-pointer transition-all duration-200 shadow-xl overflow-hidden h-full 
                ...
                >
                    <div class="p-3 bg-base-300 text-base-content text-sm font-bold truncate">
                        {themeOption.name.charAt(0).toUpperCase() + themeOption.name.slice(1)}
                    </div>
          
                    <div class="p-3 bg-base-100 flex flex-col space-y-2">
                        <div class="flex flex-wrap gap-1">
                            <div class="badge badge-xs badge-primary">P</div>
                            <div class="badge badge-xs badge-secondary">S</div>
                            <div class="badge badge-xs badge-accent">A</div>
                            <div class="badge badge-xs badge-neutral">N</div>
                        </div>
                        <button class="btn btn-primary btn-xs normal-case">Button</button>
                    </div>
                </div>
            {/each}
        </div>

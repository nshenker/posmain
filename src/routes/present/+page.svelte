<script lang='ts'>
    ...
    onMount(async () => {
        // --- MOVED BROWSER-SPECIFIC CODE HERE ---
 ...
 undefined;
                    ...
                    if (uiAmount > 0) {
                        const new_entry = {
                            ...
                            uiAmount: uiAmount,
                            mint: $selectedMint,
                            items: $currentChargeItems /* Ensure items are included in the transaction log */
                        };
                    ...
                    inventory.update(inv => {
						...
						for (const soldItem of $currentChargeItems) {
							...
							if (itemIndex > -1) {
                                ...
                                logHistory(soldItem.id, `Sale 
 (Tx: ${signatureInfo.signature.substring(0, 4)}...)`, `-${soldItem.quantity}`, newQty);
							}
                        }
                        return newInv;
					});

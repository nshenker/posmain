        ...
        intervalId = setInterval(async () => {
            ...
            try {
				...
				if (confirmedTxn) {
                    ...
                    // Method 3: Robust fallback by checking all instructions (top-level and inner)
                    if (uiAmount === 0 || isNaN(uiAmount)) {
                        
                        const isSol = $selectedMint === 'SOL';
                            ...
                            if (parsedInstruction && (parsedInstruction.type === 'transfer' || parsedInstruction.type === 'transferChecked')) {
                                ...
                                if (!isSol && currentMintInfo) {
                                    ...
                                    // 2. Fallback for simple transfer (uses raw amount string)
                                        ...
                                        const decimals = currentMintInfo.decimals || 6;
                    ...
                    if (uiAmount > 0) {
                        ...
                        const new_entry = {
                            ...
                            txid: confirmedTxn.transaction.signatures[0], // <--- RETAIN: External Solana Signature
                            uiAmount: uiAmount,
                            mint: $selectedMint,
                        ...
                        if ($selectedCustomer) {
                            ...
                            // Points are awarded on the FINAL amount spent (uiAmount) converted to USD *before* discount.
                            ...
                            if (new_entry.subtotal && new_entry.taxAmount) {
                                ...
                                usdValueForAward = new_entry.subtotal + new_entry.taxAmount - new_entry.loyaltyDiscountAmount;
                            } else {

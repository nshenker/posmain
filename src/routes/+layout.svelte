<script>
    ...
    // This handler keeps our store in sync if the user exits fullscreen with the 'Esc' key
    function handleFullscreenChange(event) {
        if ($fullScreen !== event.detail.fullscreen) {
            $fullScreen = event.detail.fullscreen;
        }
    }
</script>

<Fullscreen bind:fullscreen={$fullScreen} on:change={handleFullscreenChange}>
        ...
        <div class="fixed top-0 left-0 right-0 z-50 bg-base-100 shadow-md no-print">
            <div class="navbar px-4">
                ...
                <div class="navbar-end">
                    <ThemeSwitcher />
                    {#if browser}
                        <button class="btn btn-ghost btn-circle" on:click={() => $fullScreen = !$fullScreen}>
                            {#if !$fullScreen}

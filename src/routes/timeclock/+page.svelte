<script lang="ts">
    ...
    import { employees, timeClockEvents } from '../stores.js';
                        ...
                        {#each $employees as employee}
                            ...
                            >
                                <div class="card-body items-center justify-center text-center p-2">
                                    ...
                                    <span class="badge badge-sm {getStatusClass(employee.status)} mt-1">{employee.status || 'Clocked Out'}</span>
                                </div>
                            </button>
                        {/each}
                    </div>
            ...
            {:else}
                <div class="w-full max-w-sm mx-auto">
                     <div class="text-center">
                        <h2 class="text-2xl font-greycliffmed">Welcome, {selectedEmployee.name}</h2>
                        <p class="mb-4">Enter your PIN to continue.</p>
                    </div>
                    
                    <input type="password" class="input input-bordered w-full text-center tracking-widest text-3xl mb-4" bind:value={pin} readonly placeholder="----"/>
                    ...
                    <div class="space-y-2">
                        {#if selectedEmployee.status === 'Clocked Out' || !selectedEmployee.status}
                        ...
                        {:else if selectedEmployee.status === 'On Break'}
                            <button class="btn btn-info w-full btn-lg" on:click={() => confirmAction('End Break')}>End Break</button>

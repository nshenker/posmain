<script lang='ts'>
    ...
    function selectItem(item) {
        ...
        if (item.type === 'variable' && item.variants && item.variants.length > 0) {
            ...
            // FIX: Create a clean, explicit copy of the item to prevent reference issues
            // and ensure numbers are actual numbers (not strings)
            const cleanItem = {
                id: item.id,
                name: item.name,
                price: parseFloat(item.price), // Ensure number
                cost: parseFloat(item.cost),   // Ensure number
    ...
    function selectVariant(parentItem, variant) {
        ...
        // FIX: Create a clean object for the variant, inheriting currency from parent
        // Explicitly casting price/cost to numbers to prevent math errors later
        const itemWithVariant = {
            id: parentItem.id, // Keep parent ID for reference
            variantId: variant.id,
            name: `${parentItem.name} - ${variant.name}`,
            price: parseFloat(variant.price), // Ensure number
            cost: parseFloat(variant.cost),   // Ensure number
            currency: parentItem.currency || 'USDC', // Inherit currency
            sku: variant.sku || '',
                    ...
                    <table class="table w-full">
                        <tbody>
                            {#each selectedItemWithVariants.variants as variant}
                                ...
                                {@const isCurrencyMismatch = currentCurrency && selectedItemWithVariants.currency !== currentCurrency}
                                <tr 
                                    class={`hover ${isUnavailable || isCurrencyMismatch ? 'bg-base-200 opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                                    on:click={() => !isUnavailable && !isCurrencyMismatch && selectVariant(selectedItemWithVariants, variant)}
                                >
                                    <td>{variant.name}</td>
                                    ...
                                    <td class="text-center">
                                        <button 
                                            ...
                                            disabled={isUnavailable || isCurrencyMismatch}
                                        >
                                            {#if isCurrencyMismatch}Diff Coin
                                            {:else if !isUnavailable}Add

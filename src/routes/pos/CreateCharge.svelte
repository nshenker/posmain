 ...
 ${foundVariant.name}`,
    ...
    async function payWithCard() {
        ...
        if(chargeItems.length > 0) {
            ...
            // Handle custom amount for card payment
    ...
    function handleCardPaymentSuccess(paymentIntentId) {
        // Construct the entry using chargeForCardPayment data
        const new_entry = {
        ...
        // --- INVENTORY DEPLETION LOGIC (use items from chargeForCardPayment) ---
        if (chargeForCardPayment.items && chargeForCardPayment.items.length > 0 && !chargeForCardPayment.items[0].id.startsWith('custom-')) { // Don't deplete for custom amounts

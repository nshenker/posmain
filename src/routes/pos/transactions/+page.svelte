<script lang='ts'>
    import { successArray, mints, customers } from '../../stores.js'; // <-- CORRECTED PATH
    import { onMount, createEventDispatcher } from 'svelte';
    import { exportToCSV, exportToJSON } from '../../../utils/csv.js'; // <-- CORRECTED PATH
    import dayjs from 'dayjs';
    import { triggerPrint } from '../../printStore.js'; // <-- CORRECTED PATH
...
// State to track expanded rows
    ...
    function toggleTransaction(txn) {
        const key = txn.txnId || txn.txid; // Use new internal ID or fallback
 ...
 Items: (txn.items || []).map(item => `${item.name} (Qty: ${item.quantity || 1})`).join('; '),
            ...
            'Order Discount': (txn.orderDiscountAmount || 0).toFixed(2),
            'Order Discount Code': txn.orderDiscountCode || '',
...
<div class="container mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="card bg-base-100 shadow-xl border w-full max-w-6xl mx-auto flex-grow max-h-[85vh] overflow-y-auto flex flex-col p-0">
        
        <div class="p-4 bg-base-200 flex-shrink-0 border-b">
...
<input id="start-date" type="date" class="input input-bordered" bind:value={startDate} />
       ...
       <input id="end-date" type="date" class="input input-bordered" bind:value={endDate} 
                ...
                <div class="dropdown dropdown-end">
                        <label tabindex="0" class="btn btn-primary">Export</label>
...
<tbody>
                    ...
                    {#if filteredTransactions.length > 0}
                        {#each filteredTransactions as txn (txn.txnId || txn.txid)}
                            <tr class="hover">
       ...
       <button class="btn btn-xs btn-ghost" on:click={() => toggleTransaction(txn)}>
                                            ...
                                            {expandedTransactions[txn.txnId || txn.txid] ?
                                            '▼' : '►'}
                                        </button>
                                    {/if}
                                ...
                                <td><span class="badge badge-ghost">{txn.paymentType || getMintName(txn.mint)}</span></td>
...
N/A
                    ...
                    {#if expandedTransactions[txn.txnId || txn.txid] && txn.items && txn.items.length > 0 && !txn.items[0]?.id.startsWith('custom-')}

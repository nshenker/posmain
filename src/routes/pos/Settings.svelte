<script lang='ts'>
    ...
    import ManageCoupons from './ManageCoupons.svelte'; // NEW: Import ManageCoupons
    ...
    async function reset() {
        ...
        // This clears ALL persistent data from the browser's storage
        localStorage.clear();
        ...
        // FIX: Reset dashboard layout to the complete default set of widgets
        dashboardLayout.set({
            widgets: [
    ...
    function exportData() {
        const backupData = {
            ...
            // NOTE: cartDiscount is excluded as it is runtime state.
        };
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    coupons.set(importedData.coupons || []);
                    // IMPORTED: Coupons data
...
<div class="card w-full max-w-lg bg-base-100 shadow-xl border border-gray-200">
    <div class="card-body p-4 sm:p-8">
        <div id="settings-tabs" role="tablist" class="tabs tabs-bordered justify-center flex-shrink-0 mb-4">
            ...
            <button role="tab" class="tab" on:click={() => (activeTab = 'employees')} class:tab-active={activeTab === 
            'employees'}>Employees</button>
            <button role="tab" class="tab" on:click={() => (activeTab = 'discounts')} class:tab-active={activeTab === 'discounts'}>Discounts</button>
        </div>

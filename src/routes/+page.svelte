<script lang='ts'>
    ...
    import { showToast } from './toastStore.js'; // Ensure toastStore is imported for alerts
    ...
    onMount(async () => {
        ...
        // web3 library only on the client-side // FIX: Added missing slashes
    ...
    function validateNameAndProceed() {
        if (name.trim() === '') {
            showToast('Please enter a store name.', 'error'); // Changed alert to toast
            return;
        }
        currentStep = 2;
    }
    ...
    function validateWalletAndProceed() {
        if (pk.trim() === '') {
            showToast('Please enter a wallet address.', 'error'); // Changed alert to toast
            return;
        }

        if (!web3) {
            showToast('Page is still loading, please wait a moment.', 'info'); // Changed alert to toast
            return;
        }
        ...
        try {
            ...
            showToast('Invalid Solana wallet address. Please check and try again.', 'error'); // Changed alert to toast
        }
    }

    function save() {
        if (adminPin.trim().length < 4) {
            showToast('Your Admin PIN must be at least 4 digits.', 'error'); // Changed alert to toast
            return;
        }
        ...
        const adminUser = { id: 'admin', name: 'Admin', pin: adminPin.trim(), role: 'manager', status: 'Clocked Out', hourlyRate: 25.00 }; // Added default rate
...
<div class="navbar bg-base-100 absolute top-0 w-full z-50 justify-end shadow-md">
    <a href="/documentation" class="btn btn-ghost normal-case text-sm">Documentation</a>
</div>
...
<div class="hero min-h-screen bg-base-200">
    ...
    <div class="card-actions justify-end mt-4">
          ...
          <h2 class="card-title">Set your destination wallet</h2>
                        <p>Enter the Solana wallet address where you'd like to receive payments.
This can be changed later in the settings.</p>
                        <div class="form-control">
                 ...
                 <div class="card-actions justify-end mt-4">
                    ...
                    {:else if currentStep === 3}
                        ...
                        <p>This PIN will be used to access management features like settings and analytics.
Make it secure!</p>
                        <div class="form-control">
